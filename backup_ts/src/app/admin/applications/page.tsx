...
export default function ApplicationsPage() {
  ...
  if (isLoading) {
    return (
      <AdminLayout title="Grant Applications" description="Review and manage grant applications">
          ...
          <div className="flex space-x-1 min-w-max">
            {(['all', 'pending', 'under-review', 'approved', 'rejected'] as const).map((status) => (
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium capitalize transition-colors flex-shrink-0 ${
                  filter === status
