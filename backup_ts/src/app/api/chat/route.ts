...
const getRequestCache = new Map<string, { data: any; timestamp: number }>()
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (showLogs) console.log('🚀 GET /api/chat - Fetching chat messages...')
    ...
    if (!sessionId) {
      ...
      if (cached && (Date.now() - cached.timestamp) < CACHE_DURATION) {
        if (showLogs) console.log('📱 Serving from cache:', cacheKey)
    ...
    // Ensure connection is ready
    if (mongoose.connection.readyState !== 1) {
      if (showLogs) console.log('⏳ Waiting for database connection...')
      await new Promise((resolve) => {
        mongoose.connection.once('connected', resolve)
      })
    }
    
    if (showLogs) console.log('✅ Database connected, connection state:', mongoose.connection.readyState)
    if (showLogs) console.log('🔍 Query parameters:', { limit, status, sessionId })
    ...
    if (showLogs) console.log('📋 MongoDB query:', query)
    ...
    if (showLogs) console.log('💬 Found messages:', messages.length)
    
    // Transform the responses to match frontend interface
    const transformedMessages = messages.map(message => ({
      ...message,
      responses: message.responses ? message.responses.map((response: any) => ({
    ...
    if (showLogs) console.log('📊 Total count:', total)
    ...
    if (showLogs) console.log('📤 Sending response with', responseData.chats.length, 'chats')
...
async function sendTelegramNotification(chatMessage: { 
  priority: 'low' | 'medium' | 'high' | 'urgent',
  userInfo: { name: string, email: string },
  message: string,
  timestamp: Date
}, sessionId: string, request: NextRequest, isReply: boolean = false) {
