...
interface ChatMessage {
  ...
  timestamp: string
  sessionId: string
  status: 'new' | 'reading' | 'responded' | 'user-replied' | 'closed'
...
export default function SupportDashboard() {
  ...
  const [filter, setFilter] = useState<'all' | 'new' | 'responded' | 'user-replied' | 'closed'>('all')
  const [selectedMessage, setSelectedMessage] = useState<ChatMessage | null>(null)
  const [responseText, setResponseText] = useState('')
  const [isSubmittingResponse, setIsSubmittingResponse] = useState(false)
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; messageId?: string }>({ open: false })
  const [selectiveDeleteDialog, setSelectiveDeleteDialog] = useState<{ 
    open: boolean; 
    messageId?: string; 
    userName?: string; 
    userEmail?: string;
  ...
  const [clearDialog, setClearDialog] = useState<{ open: boolean; sessionId?: string }>({ open: false })
  ...
  const [pollingInterval, setPollingInterval] = useState(15000) // Start with 15 seconds to reduce server load
  
  // Real-time features state
  const [userPresence, setUserPresence] = useState<Map<string, any>>(new Map())
  const [readStatus, setReadStatus] = useState<Map<string, any>>(new Map())
  const [staffTyping, setStaffTyping] = useState<Map<string, boolean>>(new Map())
  const [currentStaffId] = useState(`admin_${Date.now()}`) // Unique staff ID for this session
  ...
  const showNotification = (title: string, body: string) => {
    if (notificationsEnabled && 'Notification' in window) {
      new Notification(title, {
        body,
        icon: '/images/logo.png', // Add your app icon path
  ...
  const fetchChats = useCallback(async (showNotifications = false) => {
    try {
      // Only show detailed logs when explicitly enabled
      ...
      if (response.ok) {
        ...
        if (data.chats && Array.isArray(data.chats)) {
          if (showLogs) console.log('✅ Setting chats state with', data.chats.length, 'messages')
          
          // Check for changes to reset polling interval
          const hasNewActivity = data.chats.length !== chats.length || 
            JSON.stringify(data.chats) !== JSON.stringify(chats)
          
          if (hasNewActivity) {
            lastActivityRef.current = new Date()
            setPollingInterval(15000) // Reset to moderate polling on activity
          }
          
          // Check for new user replies if this isn't the initial load
          if (showNotifications && chats.length > 0) {
            const newUserReplies: string[] = []
            
            data.chats.forEach((newChat: ChatMessage) => {
              const existingChat = chats.find(c => c._id === newChat._id)
              
              if (existingChat && newChat.responses && existingChat.responses) {
                // Check if there are new responses
                const newResponseCount = newChat.responses.length
                const oldResponseCount = existingChat.responses.length
                
                if (newResponseCount > oldResponseCount) {
                  // Check if the latest responses are user replies
                  const newResponses = newChat.responses.slice(oldResponseCount)
                  const hasUserReply = newResponses.some(response => response.isUserReply)
                  
                  if (hasUserReply) {
                    newUserReplies.push(newChat._id)
                    
                    // Show notification for new user reply
                    const userName = newChat.userInfo?.name || 'User'
                    success('New Reply', `${userName} has replied to their message`)
                    
                    // Show browser notification
                    showNotification(
                      'New User Reply',
                      `${userName} has replied: "${newResponses.find(r => r.isUserReply)?.message?.substring(0, 50) || 'New message'}..."`
                    )
            ...
            // Update new replies set
            if (newUserReplies.length > 0) {
              setNewReplies(prev => {
                const updated = new Set(prev)
                newUserReplies.forEach(id => updated.add(id))
                return updated
              })
              
              // If the currently selected message has a new reply, refresh it
              if (selectedMessage && newUserReplies.includes(selectedMessage._id)) {
                const updatedMessage = data.chats.find((c: ChatMessage) => c._id === selectedMessage._id)
                if (updatedMessage) {
  ...
  }, [chats, selectedMessage, success, error])

  // Conservative polling with much longer intervals to prevent server overload
  const scheduleNextPoll = useCallback(() => {
    ...
    // Much more conservative intervals
    if (timeSinceLastActivity > 300000) { // 5 minutes
      interval = Math.min(interval * 1.5, 120000) // Max 2 minutes
      setPollingInterval(interval)
    } else if (timeSinceLastActivity > 60000) { // 1 minute
      interval = Math.min(interval * 1.2, 60000) // Max 1 minute
      setPollingInterval(interval)
    } else if (timeSinceLastActivity < 10000) { // Recent activity
      interval = Math.max(interval * 0.9, 10000) // Min 10 seconds - much slower than before
      setPollingInterval(interval)
    }
  ...
  }, [fetchChats, pollingInterval, pollingPaused, setPollingInterval])
  ...
  useEffect(() => {
    ...
    if (chats.length > 0) {
      ...
      // Poll presence every 30 seconds (less frequent than chat polling)
  ...
  // Pause/resume polling when window focus changes
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        setPollingInterval(30000) // Reset to slower polling when returning
        scheduleNextPoll()
      }
    }
  ...
  const markMessageAsRead = async (messageId: string) => {
    try {
      ...
      setReadStatus(prev => new Map(prev).set(messageId, {
        isRead: true,
        readBy: currentStaffId,
        readAt: new Date()
      }))
    } catch (error) {
  ...
  const updatePresence = async (sessionIds: string[]) => {
    try {
  ...
  const assignConversation = async (messageId: string) => {
    try {
      await fetch('/api/chat/status', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Clear new reply indicators when a message is selected
  const handleMessageSelect = (message: ChatMessage) => {
    ...
    // Auto-scroll to bottom of conversation after a short delay
    setTimeout(() => {
      conversationEndRef.current?.scrollIntoView({ behavior: 'smooth' })
    }, 100)
  }

  // Auto-scroll when new replies are added to the currently selected conversation
  useEffect(() => {
    if (selectedMessage && conversationEndRef.current) {
      conversationEndRef.current.scrollIntoView({ behavior: 'smooth' })
    }
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    if (message) {
      setSelectiveDeleteDialog({ 
        open: true, 
        messageId,
        userName: message.userInfo.name,
        userEmail: message.userInfo.email,
        type: 'message'
      })
    }
  }

  const handleSelectiveDelete = async (messageId: string, deleteType: 'admin-only' | 'both') => {
    try {
  ...
  const clearConversation = async (sessionId: string) => {
    setClearDialog({ open: true, sessionId })
  }

  // Response deletion functions
  const deleteResponse = async (responseId: string, responseSender: string) => {
    // For now, use selective delete for responses too
    setSelectiveDeleteDialog({ 
      open: true, 
      messageId: responseId,
      userName: responseSender,
      userEmail: 'Response Message',
      type: 'response'
    })
  }

  const handleResponseSelectiveDelete = async (responseId: string, deleteType: 'admin-only' | 'both') => {
    try {
      const response = await fetch(`/api/chat/responses/${responseId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        const result = await response.json()
        success('Success', result.message)
        fetchChats() // Refresh to show updated conversation
      } else {
        const error = await response.json()
  ...
  const permanentDeleteResponse = async (responseId: string) => {
    try {
      const response = await fetch(`/api/chat/responses/${responseId}`, {
        method: 'POST', // Using POST for permanent delete
      })

      if (response.ok) {
        success('Success', 'Response deleted permanently')
        fetchChats() // Refresh to show updated conversation
      } else {
      ...
      console.error('Error deleting response:', err)
      error('Error', 'Error deleting response')
    }
  }
  ...
  const confirmDeleteAllMessages = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json()
        success('Success', result.message || 'All messages deleted successfully')
        setSelectedMessage(null)
        setChats([])
        // Refresh the chat list to ensure it's empty
        await fetchChats()
      } else {
        const errorData = await response.json()
        throw new Error(errorData.error || 'Failed to delete all messages')
      }
    } catch (err) {
  ...
  const submitResponse = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Response successful in ${processingTime}ms:`, responseData)
        success('Success', `Response sent successfully (${processingTime}ms)`)
        setResponseText('')
        
        // Optimistic update: immediately update the local state instead of refetching
        if (selectedMessage && responseData.response) {
          setChats(prevChats => 
            prevChats.map(chat => 
              chat._id === selectedMessage._id 
                ? {
                    ...chat,
                    responses: [
                      ...(chat.responses || []),
                      {
                        ...
                        supportAgent: responseData.response.supportAgent,
                        isUserReply: false
                      }
                    ],
                    status: 'responded'
          ...
          // Update selectedMessage as well
        ...
        // Schedule a background refresh after a short delay instead of immediate blocking refresh
        setTimeout(() => {
          fetchChats(false) // Silent refresh to sync with server
      ...
      error('Error', `Error sending response: ${err instanceof Error ? err.message : 'Unknown error'}`)
    } finally {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'user-replied': return 'bg-orange-100 text-orange-800 border-orange-200'
  ...
  return (
          ...
          <div className="flex flex-col space-y-2 sm:flex-row sm:items-center sm:space-x-4 sm:space-y-0 text-sm">
            ...
            <button
              onClick={() => {
                if ('Notification' in window) {
                  ...
                  className={`flex items-center justify-center space-x-2 px-3 py-1 rounded-lg border transition-colors ${
            ...
            <div className="flex space-x-1 mb-4 overflow-x-auto pb-2 scrollbar-hide">
              <div className="flex space-x-1 min-w-max">
                {(['all', 'new', 'responded', 'user-replied', 'closed'] as const).map((status) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium capitalize transition-colors relative flex-shrink-0 ${
                        filter === status
                    ...
                    >
                      ...
                      {newRepliesCount > 0 && (
                        <span className="absolute -top-1 -right-1 bg-green-500 text-white text-xs font-bold min-w-[16px] h-[16px] rounded-full flex items-center justify-center animate-pulse">
                          {newRepliesCount}
                        </span>
                      )}
                    </button>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-gray-50 rounded-lg border space-y-2 sm:space-y-0">
              ...
              <div className="flex gap-2 justify-center sm:justify-end">
                <button
                  onClick={() => fetchChats()}
                  className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors text-xs sm:text-sm"
                >
                  Refresh
                </button>
                <button
                  onClick={deleteAllMessages}
                  className="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 transition-colors text-xs sm:text-sm"
                >
                  Clear All
                </button>
              </div>
              ...
              {filteredChats.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[500px] overflow-y-auto">
                  {filteredChats.map((chat) => (
                    <div
                      key={chat._id}
                      onClick={() => handleMessageSelect(chat)}
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-3 mb-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 relative ${
                          newReplies.has(chat._id) ? 'bg-green-100' : 'bg-blue-100'
                        }`}>
                          <span className={`text-sm font-medium ${
                            newReplies.has(chat._id) ? 'text-green-600' : 'text-blue-600'
                          }`}>
                            {chat.userInfo.name.charAt(0).toUpperCase()}
                          </span>
                          {/* Online Status Indicator */}
                          {userPresence.get(chat.sessionId)?.userOnline && (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              ...
                              {/* Read Status */}
                              {readStatus.get(chat._id)?.isRead && (
                              ...
                              {/* Typing Indicator */}
                              {userPresence.get(chat.sessionId)?.userActivity?.typing && (
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-500 gap-2">
                        ...
                        {chat.responses && chat.responses.length > 0 && (
                          <span className="bg-green-100 text-green-600 px-2 py-0.5 rounded-full flex-shrink-0">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                    ...
                    {selectedMessage.responses && selectedMessage.responses.length > 0 && (
                      selectedMessage.responses.map((response, index) => {
                        ...
                        const responseId = (response as any)._id || `response-${index}` // Get response ID
                        return (
                          <div key={index} className={`flex ${response.isUserReply ? 'justify-start' : 'justify-end'} group`}>
                            <div className="max-w-[85%] sm:max-w-[70%] relative">
                              <div className={`rounded-lg p-3 shadow-sm relative ${
                                ...
                                {response.isUserReply && isRecent && (
                                  <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full animate-ping"></div>
                                )}
                                <p className="break-words">{response.message}</p>
                              </div>
                              <div className={`text-xs mt-1 px-1 flex items-center flex-wrap ${
                                response.isUserReply ? 'justify-start text-gray-600' : 'justify-end text-gray-500'
                              }`}>
                                {response.isUserReply && (
                              ...
                              <button
                                onClick={() => deleteResponse(responseId, response.isUserReply ? (response.userInfo?.name || 'User') : response.supportAgent)}
                                className={`absolute -top-2 w-6 h-6 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center hover:bg-red-600 ${
                                  response.isUserReply ? '-right-2' : '-left-2'
                                }`}
    ...
    <SelectiveDeleteDialog
      open={selectiveDeleteDialog.open}
      onOpenChange={(open) => setSelectiveDeleteDialog({ open })}
      messageId={selectiveDeleteDialog.messageId || ''}
      userName={selectiveDeleteDialog.userName || ''}
      userEmail={selectiveDeleteDialog.userEmail || ''}
    ...
    <ConfirmDialog
      ...
      onOpenChange={(open) => setClearDialog({ open })}
      title="Clear Conversation"
      description="Are you sure you want to clear this entire conversation? This action cannot be undone and will remove all messages in this conversation thread."
    ...
    <DoubleConfirmDialog
      ...
      secondaryDescription="This will permanently delete ALL chat messages and conversations. Are you absolutely sure?"
      confirmText="Continue"
      secondaryConfirmText="Yes, Delete Everything"
