...
export async function POST(request) {
    try {
        ...
        if (chatId) {
            ...
            if (!sendResult.ok) {
                return NextResponse.json({ success: false, error: `Failed to send test message: ${sendResult.description}` }, { status: 400 });
            }
        ...
        return NextResponse.json({ success: false, error: 'Failed to test Telegram connection' }, { status: 500 });
    }
}
