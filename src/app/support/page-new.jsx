...
export default function SupportDashboard() {
    ...
    const [clearConversationDialog, setClearConversationDialog] = useState({ open: false });
    ...
    return (<div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          ...
          <div className="flex space-x-1 mb-6">
            {['all', 'new', 'responded', 'closed'].map((status) => (<button key={status} onClick={() => setFilter(status)} className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${filter === status
          ...
          <div className="flex justify-between items-center mb-6 p-4 bg-gray-50 rounded-lg border">
            <div className="flex items-center gap-4">
              ...
              {selectedMessage && (<div className="flex gap-2">
                  <button onClick={() => deleteMessage(selectedMessage._id)} className="px-3 py-1 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors text-sm flex items-center gap-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                    </svg>
                    Delete Message
                  </button>
                  <button onClick={() => clearConversation(selectedMessage.sessionId)} className="px-3 py-1 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 transition-colors text-sm flex items-center gap-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex gap-2">
              <button onClick={deleteAllMessages} className="px-3 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm flex items-center gap-1">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-4">
            {filteredChats.length === 0 ? (<div className="text-center py-8 text-gray-500">
                No chat messages found for the selected filter.
              </div>) : (filteredChats.map((chat) => (<div key={chat._id} onClick={() => setSelectedMessage(chat)} className={`p-4 border rounded-lg cursor-pointer transition-colors hover:bg-gray-50 ${selectedMessage?._id === chat._id ? 'ring-2 ring-blue-500 bg-blue-50' : ''}`}>
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    ...
                    {chat.responses && chat.responses.length > 0 && (<span className="ml-2">• {chat.responses.length} response(s)</span>)}
                  </div>
                </div>)))}
          </div>

          {/* Selected Message Details */}
          {selectedMessage && (<div className="mt-8 p-6 bg-gray-50 rounded-lg border">
              ...
              {selectedMessage.responses && selectedMessage.responses.length > 0 && (<div>
                  <span className="font-medium text-gray-700">Responses:</span>
                  <div className="mt-2 space-y-2">
                    {selectedMessage.responses.map((response, index) => (<div key={index} className="p-3 bg-white rounded border">
      ...
      <ConfirmDialog open={clearConversationDialog.open} onOpenChange={(open) => setClearConversationDialog({ open })} title="Clear Conversation" description="Are you sure you want to clear this entire conversation? This action cannot be undone." confirmText="Clear Conversation" variant="warning" onConfirm={confirmClearConversation}/>

      {/* Delete All Messages Dialog */}
      <DoubleConfirmDialog open={deleteAllDialog} onOpenChange={setDeleteAllDialog} title="Delete ALL Messages" description="Are you sure you want to delete ALL chat messages? This action cannot be undone and will permanently remove all chat data from the system." secondDescription="This will permanently delete ALL chat messages and conversations. Are you absolutely sure?" confirmText="Delete All Messages" onConfirm={confirmDeleteAllMessages}/>
    </div>);
}
