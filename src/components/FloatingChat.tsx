...
export default function FloatingChat() {
  ...
  const [lastCheckedForResponses, setLastCheckedForResponses] = useState<Date | null>(null)
  const [hasNewMessages, setHasNewMessages] = useState(false)

  // Real-time presence tracking
  const sendPresenceUpdate = useCallback(async (action: string = 'online') => {
    if (!sessionId) return
  ...
  // Send presence updates on key interactions
  useEffect(() => {
    if (sessionId && hasUserInfo) {
      sendPresenceUpdate('online')
      
      // Update presence when window closes
  ...
  }, [sessionId, hasUserInfo, sendPresenceUpdate])
  ...
  const _toast = useToast() as any
  const { success, error: showError, info } = _toast

  // Notification helpers must be declared before any effects that call them
  // to avoid module initialization ordering issues in the compiled client bundle.
  async function requestNotificationPermission(): Promise<NotificationPermission> {
    ...
    if ('Notification' in window && Notification.permission === 'default') {
      ...
      if (permission === 'granted') {
        showNotification(
          'Impact Grant Solutions Chat',
          'Notifications enabled! We\'ll notify you when our support team responds.'
        )
  ...
  useEffect(() => {
    ...
    const savedHasUserInfo = localStorage.getItem('chatHasUserInfo')
    ...
    if (savedMessages) {
      try {
        ...
        const restoredMessages = messagesData.map((msg: any) => ({
          ...msg,
        ...
        const hasUserMessages = restoredMessages.some((msg: Message) => msg.isUser)
  ...
  // Check for messages when tab becomes visible again or window gets focus
  useEffect(() => {
    const handleVisibilityChange = async () => {
      if (!document.hidden && hasUserInfo && sessionId) {
    ...
    const handleWindowFocus = async () => {
      if (hasUserInfo && sessionId) {
        // Window got focus, check for new messages
        await checkForNewMessages()
      }
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.newResponses && data.newResponses.length > 0) {
            const supportMessages: Message[] = data.newResponses.map((resp: any) => ({
              id: resp._id,
  ...
  }, [hasUserInfo, sessionId, lastCheckedForResponses, isOpen])

  // Check for new responses from support staff with conservative polling
  useEffect(() => {
    if (!hasUserInfo || !sessionId) return
        ...
        if (response.ok) {
          ...
          if (data.newResponses && data.newResponses.length > 0) {
            ...
            if (!isOpen && supportMessages.length > 0) {
              ...
              // Show browser notification (inline to avoid cross-binding init order issues)
  ...
  }, [hasUserInfo, sessionId, isOpen, hasNewMessages]) // Removed lastCheckedForResponses to prevent excessive re-runs
  ...
  const handleTyping = useCallback(() => {
    ...
    typingTimeoutRef.current = setTimeout(() => {
      setIsTyping(false)
      sendPresenceUpdate('stop_typing')
    }, 2000)
  }, [isTyping, sendPresenceUpdate])
  ...
  // (Notification helpers declared earlier to ensure correct initialization order)
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-24 right-2 left-2 sm:left-auto sm:right-6 sm:w-96 h-[500px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col z-50 overflow-hidden">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full hover:bg-white/20 flex items-center justify-center transition-colors"
                title="Reset session (clear everything)"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {!hasUserInfo ? (
            <div className="flex-1 p-6 flex items-center justify-center">
              <form onSubmit={handleSubmitUserInfo} className="w-full space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:shadow-md transition-all duration-300"
                >
                  Start Chat
                </button>
              </form>
                    ...
                    <div className={`flex items-start gap-2 max-w-[80%] ${message.isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      {message.isUser && message.id !== '1' && (
                        <button
                          ...
                          className="w-6 h-6 rounded-full hover:bg-red-100 flex items-center justify-center transition-colors text-gray-400 hover:text-red-600 mt-1"
              ...
              {messages.length > 1 && (
                <div className="px-4 py-2 border-t border-gray-100 bg-gray-50">
                  <div className="flex gap-2 justify-center">
                    ...
                    <button
                      onClick={clearChatSession}
                      className="text-xs px-3 py-1 bg-orange-100 text-orange-700 rounded-full hover:bg-orange-200 transition-colors flex items-center gap-1"
                    >
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                      </svg>
              ...
              <div className="p-4 border-t border-gray-200">
                <form onSubmit={sendMessage} className="flex gap-2">
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:shadow-md transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open: boolean) => setDeleteMessageDialog({ open })}
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open: boolean) => setClearMessagesDialog(open)}
        title="Clear All Messages"
        description="Are you sure you want to clear all messages? This will keep your session but remove all conversation history." 
