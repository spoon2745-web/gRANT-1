...
import { Calendar, MapPin, AlertTriangle, ExternalLink, Award, Globe2, Sparkles } from 'lucide-react';
const regions = [
    {
...
export default function GrantSchedule() {
    ...
    return (<section className="relative py-24 bg-gradient-to-br from-white via-gray-50/50 to-blue-50/30 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8 mb-12">
          {regions.map((region, index) => (<Card key={region.name} className={`group relative overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-105 cursor-pointer backdrop-blur-sm ${region.status === 'open'
                ...
                : region.status === 'upcoming'
                    ...
                    : 'bg-gradient-to-br from-white to-gray-50 border border-gray-200/50'}`} onClick={() => setSelectedRegion(region)}>
              {/* Glow effect for open applications */}
              {region.status === 'open' && (<div className="absolute inset-0 bg-gradient-to-r from-emerald-400/20 to-green-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>)}
              
              <CardHeader className="relative z-10 space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-xl ${region.status === 'open' ? 'bg-emerald-100' :
                region.status === 'upcoming' ? 'bg-amber-100' : 'bg-gray-100'}`}>
                      <MapPin className={`w-5 h-5 ${region.status === 'open' ? 'text-emerald-600' :
              ...
              <CardContent className="relative z-10 space-y-4">
                ...
                {region.status === 'open' && (<div className="pt-4">
              ...
              {region.status === 'open' && (<div className="absolute top-4 right-4 opacity-50 group-hover:opacity-100 transition-opacity">
