    ...
    return (<section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          {regions.map((region, index) => (<Card key={region.name} className={`relative border-2 shadow-lg hover:shadow-xl transition-shadow duration-300 ${region.status === 'open'
              ...
              <CardHeader className="space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-xl ${region.status === 'open' ? 'bg-green-100' :
                region.status === 'upcoming' ? 'bg-amber-100' : 'bg-gray-100'}`}>
                      <MapPin className={`w-5 h-5 ${region.status === 'open' ? 'text-green-600' :
