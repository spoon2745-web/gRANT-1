...
export default function SelectiveDeleteDialog({ open, onOpenChange, messageId, userName, userEmail, onConfirm }) {
    const [deleteType, setDeleteType] = useState('admin-only');
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          <div className="space-y-3">
            <label className="flex items-start space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 transition-colors">
              <input type="radio" name="deleteType" value="admin-only" checked={deleteType === 'admin-only'} onChange={(e) => setDeleteType(e.target.value)} className="mt-1"/>
            ...
            <label className="flex items-start space-x-3 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 transition-colors">
              <input type="radio" name="deleteType" value="both" checked={deleteType === 'both'} onChange={(e) => setDeleteType(e.target.value)} className="mt-1"/>
              <div className="flex-1">
          ...
          {deleteType === 'both' && (<div className="bg-red-50 border border-red-200 rounded-lg p-3">
        ...
        <DialogFooter className="gap-2">
          ...
          <Button variant={deleteType === 'both' ? 'destructive' : 'default'} onClick={handleConfirm} disabled={isConfirming} className={deleteType === 'admin-only' ? 'bg-orange-600 hover:bg-orange-700' : ''}>
            {isConfirming ? (<>
